...
- The SPA reads from core's REST API (`/{prefix}/api/...`). Run a host app with both `ichava/core` and `ichava/icon-browser` installed for end-to-end testing.
- Class short names use the constant convention: `IconBrowserServiceProvider`, `IchavaTestIconComponent`, etc. Browser-only classes live under the `Simtabi\Laranail\Ichava\IconBrowser\` namespace.
- Conventional Commits (`feat:`, `fix:`, `docs:`, `refactor:`, `perf:`, `test:`).

## Reporting issues

Use the [issue tracker](https://github.com/ichava/icon-browser/issues). Security issues go to **security@simtabi.com** privately.

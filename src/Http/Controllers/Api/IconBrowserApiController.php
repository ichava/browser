...
final class IconBrowserApiController extends BaseApiController
{
    ...
    public function svg(Request $request, int $id): Response
    {
        try {
            ...
            // Defense in depth. `Icon::svg_content` sanitises through
            // SvgProcessingService before caching, so this is the second layer rather
            // than the only one -- which is what this comment used to assert while the
            // accessor was a bare File::get(). Browsers execute scripts in an SVG
